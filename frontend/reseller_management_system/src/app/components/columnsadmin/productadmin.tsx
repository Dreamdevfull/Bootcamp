  ...
  return (
    <div className="flex items-center justify-center gap-2">
      <button 
        ...
        className='text-[#ffffff] border bg-[#1b9e75] hover:bg-[#1a6b5a] hover:text-[#FAEEDA] rounded-lg px-5 py-2 cursor-pointer transition'
      >
        แก้ไข
      </button>
      <PopEditProducts 
...
export const productColumns = (onSuccess: () => void): ColumnDef<ProductType>[] => [
  ...
  },
  {
    ...
    header: () => <div className="text-center">ราคาทุน</div>, 
    cell: ({ row }) => (
      <div className="text-center">฿{Number(row.getValue("cost_price")).toLocaleString()}</div>
    ),
  },
  {
    accessorKey: "min_price",
    header: () => <div className="text-center">ราคาขายขั้นต่ำ</div>, 
    cell: ({ row }) => (
      <div className="text-center">฿{Number(row.getValue("min_price")).toLocaleString()}</div>
    ),
  },
  {
    accessorKey: "stock",
    header: () => <div className="text-center">จํานวน</div>, 
    cell: ({ row }) => (
      <div className="text-center">{row.getValue("stock")} ชิ้น</div>
    ),
  },
  {
    id: "actions",
    ...
    cell: ({ row }) => (
      <div className="flex items-center justify-center gap-2">
        ...
        <ActionCell 
            ...
            cost_price={row.original.cost_price} 
