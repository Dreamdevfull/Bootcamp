...
const CardWallet = ({ data, loading }: CardWalletProps) => {
  ...
  if (loading) {
    return (
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4 mx-4 sm:mx-8 mt-5">
        <div className="bg-[#1A6B5A]/20 animate-pulse rounded-2xl h-[180px]"></div>
        <div className="bg-gray-100 animate-pulse rounded-2xl h-[180px]"></div>
      </section>
    )
  }
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 gap-4 mx-4 sm:mx-8 mt-5">
      {/* กำไรสะสมทั้งหมด */}
      <div className="bg-[#1A6B5A] rounded-2xl p-6 text-white shadow-sm transition-transform active:scale-[0.98]">
        ...
        <p className="text-[#9FE1CB] text-xs sm:text-sm mt-3 opacity-90">
          รายได้จากส่วนต่างราคาขาย - ต้นทุน
        </p>
      </div>

      {/* สถานะยอดเงิน */}
      <div className="bg-white border border-[#D3D1C7] rounded-2xl p-6 shadow-sm">
        <p className="font-bold text-[#2C2C2A] text-lg mb-4">สถานะยอดเงิน</p>
        
        <div className="space-y-1">
          <div className="flex justify-between items-center py-3 border-b border-[#D3D1C7]">
            <p className="text-[#888780] text-sm sm:text-base">รับยอดเข้าแล้ว</p>
            <p className="font-bold text-[#085041] text-lg">
